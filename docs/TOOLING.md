...
- Branch and merge-request pipelines run `verify_windows`, which checks version surfaces and runs `just ci-remote-fast`.
- Tag pipelines additionally package `target/package/*.crate`, publish to crates.io using `CARGO_REGISTRY_TOKEN`, upload the crate package to GitLab generic packages, and create/update the GitLab release.
