...
cargo test --all-targets --locked --features _test_helpers
```

## Local Release Commands

- `just verify-version` - checks `Cargo.toml`, `Cargo.lock`, and tag/version alignment.
- `just bump-version-dry-run X.Y.Z` - previews the semver bump and generated changelog entry.
- `just bump-version X.Y.Z` - updates version surfaces and `CHANGELOG.md`, commits, tags `vX.Y.Z`, and pushes the branch plus tag.
...
- `just release-gate` - runs full local release gate (version surface check + crates.io version check + `ci-deep` + package listing + dry-run).
...
- Protected tag pipelines use the Windows protected runner boundary to run `verify_release_windows` including `just release-version-check`, package `target/package/*.crate`, emit `*.crate.sha256` plus `cargo-metadata.json`, publish to crates.io using the protected `CARGO_REGISTRY_TOKEN`, upload the crate package to GitLab generic packages, and create/update the GitLab release.
- Changelog entries are generated by `scripts/bump-version.ps1` from commits since the previous `vX.Y.Z` tag and inserted under `CHANGELOG.md`'s `Unreleased` heading.
